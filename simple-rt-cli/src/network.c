...
#define ACC_ID_FROM_ADDR(addr) ((addr)&0xff)
...
accessory_id_t get_acc_id_from_packet(const uint8_t* data, size_t size, bool dst_addr) {
    ...
    /* dest ip addr in LE */
    addr = (uint32_t)(data[addr_offset + 0] << 24) | (uint32_t)(data[addr_offset + 1] << 16) |
           (uint32_t)(data[addr_offset + 2] << 8) | (uint32_t)(data[addr_offset + 3] << 0);
...
static bool iface_up(const char* dev) {
    ...
    snprintf(net_addr_str, sizeof(net_addr_str), "%s", inet_ntoa(*(struct in_addr*)&net_addr));

    snprintf(host_addr_str, sizeof(host_addr_str), "%s", inet_ntoa(*(struct in_addr*)&host_addr));

    snprintf(cmd, sizeof(cmd), "%s %s start %s %s %s %u %s %s\n", IFACE_UP_SH_PATH, PLATFORM, dev, net_addr_str,
             host_addr_str, mask, config->nameserver, config->interface);
...
static bool iface_down(void) {
    ...
    snprintf(cmd, sizeof(cmd), "%s %s stop", IFACE_UP_SH_PATH, PLATFORM);
...
char* fill_serial_param(char* buf, size_t size, accessory_id_t id) {
    ...
    snprintf(buf, size, "%s,%s", inet_ntoa(*(struct in_addr*)&addr), config->nameserver);
