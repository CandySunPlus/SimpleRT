...
static inline void dump_addr_info(uint32_t addr, size_t size) {
  ...
  printf("packet size = %zu, dest addr = %s, device id = %d\n", size,
         inet_ntoa(*(struct in_addr *)&tmp), addr & 0xff);
}
...
accessory_id_t get_acc_id_from_packet(const uint8_t *data, size_t size,
  ...
  /* dest ip addr in LE */
  addr = (uint32_t)(data[addr_offset + 0] << 24) |
...
static bool iface_up(const char *dev) {
  ...
  snprintf(cmd, sizeof(cmd), "%s %s start %s %s %s %u %s %s\n",
           ...
           config->nameserver, config->interface);
...
char *fill_serial_param(char *buf, size_t size, accessory_id_t id) {
  ...
  snprintf(buf, size, "%s,%s", inet_ntoa(*(struct in_addr *)&addr),
           config->nameserver);
