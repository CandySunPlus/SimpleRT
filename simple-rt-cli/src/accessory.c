...
static bool is_accessory_id_valid(accessory_id_t id) { return id && id < ARRAY_SIZE(acc_list); }
...
static void accessory_worker_proc(accessory_t* acc) {
    ...
    while (acc->is_running) {
        if ((nread = read_usb_packet(acc->handle, acc->ep_in, acc_buf, sizeof(acc_buf))) > 0) {
...
int send_accessory_packet(const uint8_t* data, size_t size, accessory_id_t id) {
    accessory_t* acc;
