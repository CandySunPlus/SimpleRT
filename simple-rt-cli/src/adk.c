...
static bool is_accessory_present(struct libusb_device* dev) {
    static const uint16_t aoa_pids[] = {
        AOA_ACCESSORY_PID, AOA_ACCESSORY_ADB_PID,   AOA_AUDIO_PID,
        AOA_AUDIO_ADB_PID, AOA_ACCESSORY_AUDIO_PID, AOA_ACCESSORY_AUDIO_ADB_PID,
    };
...
accessory_t* probe_usb_device(struct libusb_device* dev, gen_new_serial_str_cb gen_new_serial_str) {
    int ret = 0;
    ...
    ret = libusb_control_transfer(handle, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_VENDOR, AOA_GET_PROTOCOL, 0, 0,
    ...
    for (struct acc_control_params_t* acp = acc_control_params; acp->str != NULL; acp++) {
        const char* data = acp->data ? acp->data : "start accessory";
        uint16_t data_len = acp->data ? strlen(acp->data) + 1 : 0;

        printf(" sending %s: %s\n", acp->str, data);

        ret = libusb_control_transfer(handle, acp->request_type, acp->bRequest, acp->wValue, acp->wIndex,
                                      (uint8_t*)acp->data, data_len, acp->timeout);
        if (ret < 0) {
            goto error;
        }
    }
...
ssize_t read_usb_packet(struct libusb_device_handle* handle, uint8_t ep, uint8_t* data, size_t size) {
    int ret;
    int transferred;
...
ssize_t write_usb_packet(struct libusb_device_handle* handle, uint8_t ep, const uint8_t* data, size_t size) {
    int ret;
    int transferred;

    while (true) {
        ret = libusb_bulk_transfer(handle, ep, (uint8_t*)data, size, &transferred, ACC_TIMEOUT);
        if (ret < 0) {
            if (ret != LIBUSB_ERROR_TIMEOUT) {
