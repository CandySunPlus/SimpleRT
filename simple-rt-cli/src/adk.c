...
static uint16_t get_accessory_endpoints(struct libusb_device *dev) {
  ...
  const struct libusb_endpoint_descriptor *endpoint_in =
      &interface->endpoint[0];
  const struct libusb_endpoint_descriptor *endpoint_out =
...
accessory_t *probe_usb_device(struct libusb_device *dev,
  ...
  for (struct acc_control_params_t *acp = acc_control_params; acp->str != NULL;
       acp++) {
    ...
    ret = libusb_control_transfer(handle, acp->request_type, acp->bRequest,
                                  acp->wValue, acp->wIndex,
...
ssize_t read_usb_packet(struct libusb_device_handle *handle, uint8_t ep,
  ...
  while (true) {
    ret =
        libusb_bulk_transfer(handle, ep, data, size, &transferred, ACC_TIMEOUT);
    if (ret < 0) {
...
ssize_t write_usb_packet(struct libusb_device_handle *handle, uint8_t ep,
  ...
  while (true) {
    ret = libusb_bulk_transfer(handle, ep, (uint8_t *)data, size, &transferred,
                               ACC_TIMEOUT);
    if (ret < 0) {
      if (ret == LIBUSB_ERROR_TIMEOUT) {
        continue;
