...
static int hotplug_callback(struct libusb_context* ctx, struct libusb_device* dev, libusb_hotplug_event event,
                            void* arg) {
...
int main(int argc, char* argv[]) {
    ...
    while ((rc = getopt(argc, argv, "hdi:n:")) != -1) {
        ...
        case 'h':
            printf("usage: sudo %s [-h] [-i interface] [-n nameserver|\"local\" ]\n"
                   ...
                   argv[0], config->interface, config->nameserver);
            return EXIT_SUCCESS;
        case 'd':
    ...
    rc = libusb_hotplug_register_callback(NULL, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED, LIBUSB_HOTPLUG_ENUMERATE,
